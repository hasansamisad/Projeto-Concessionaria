...
<form action="?page=salvar-venda" method="POST">
            ...
            <?php
                ...
                if($res_cliente->num_rows > 0){
                    while($row_cliente = $res_cliente->fetch_object()){
                        // Exibe nome e CPF para facilitar a identificação
                        print "<option value='{$row_cliente->id_cliente}'>{$row_cliente->nome_cliente} (CPF: {$row_cliente->cpf_cliente})</option>";
                    }
                } else {
    ...
    <div class="mb-3">
        ...
        <select name="modelo_id_modelo" class="form-control" required>
            ...
            <?php
                // Faz um JOIN com 'marca' para exibir o nome completo: [Marca] - [Modelo]
                ...
                if($res_mod->num_rows > 0){
                    while($row_mod = $res_mod->fetch_object()){
                        print "<option value='{$row_mod->id_modelo}'>{$row_mod->nome_marca} - {$row_mod->nome_modelo} ({$row_mod->id_modelo})</option>";
                    }
                } else {
    ...
    <div class="mb-3">
        ...
        <input type="number" step="0.01" name="valor_venda" class="form-control" placeholder="Ex: 55000.00" required>
    </div>
